...
# Alleen de genregels selecteren
gff_gene <- gff %>% filter(type == "gene")

# 'type' aanpassen naar 'exon' zodat featureCounts het gaat accepteren
gff_gene$type <- "exon"
...
# definieer een vector met de namen van BAM-bestanden. Elke BAM heeft reads van een RNA-seq-experiment (bijv. behandeld vs. controle).
...
# Resultaten opslaan in een gewenst bestand
# Bij het opslaan van je tabel ga je opnieuw je pad instellen met `setwd()` of het gehele pad waar je de tabel wilt opslaan opgeven in de code.
...
# Alternatieve plot zonder p-waarde cutoff (hier zijn alle genen zichtbaar)
EnhancedVolcano(resultaten,
                lab = rownames(resultaten),
                x = 'log2FoldChange',
                y = 'padj',
...
pathview(
  ...
  gene.idtype = "SYMBOL",     # stel in dat het KEGG-ID's zijn
